...
class TrackerRepositoryImpl(
    ...
    override suspend fun searchFood(
        ...
        return try {
            ...
            Result.success(
                searchDto.products
                    .filter {
                        val calculatedCalories = (it.nutriments.carbohydrates100g?.times(4f)?: 0.0) +
                                (it.nutriments.proteins100g?.times(4f)?: 0.0) +
                                (it.nutriments.fat100g?.times(9f)?: 0.0)
                        val lowerBound = calculatedCalories * 0.99f
                        val upperBound = calculatedCalories * 1.01f
                        if (it.nutriments.energyKcal100g == null) 0.0 in (lowerBound..upperBound)
                        else it.nutriments.energyKcal100g in (lowerBound..upperBound)
                    }
    ...
    override fun getFoodByDate(localDate: LocalDate): Flow<List<TrackedFood>> {
        return dao.getFoodByDate(localDate.dayOfMonth, localDate.monthValue, localDate.year)
            .map { entities ->
                entities.map {
                    it.toTrackedFood()
                }
